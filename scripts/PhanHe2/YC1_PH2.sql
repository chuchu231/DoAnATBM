...
AS
    v_role VARCHAR2(30);
BEGIN    
    IF (USERNAME IN ('admin', 'ADMIN')) THEN 
        v_role := 'ADMIN';
    ELSE
...
SELECT GRANTED_ROLE 
        FROM DBA_ROLE_PRIVS
        WHERE GRANTEE = 'SV03' AND GRANTED_ROLE != 'CONNECT';
REVOKE SINHVIEN FROM SV03;
GRANT RL_SINHVIEN TO SV03;
set serveroutput on;
DECLARE
    v_role VARCHAR2(30);
BEGIN
    v_role := GETROLE_BYUSERNAME('NS013');
...
BEGIN 
    ...
    IF sys_context('my_context', 'vpd_bypass') = 'TRUE' THEN
        RETURN '1=1'; 
    ELSE
        IF (ROLE IN ('RL_TRUONGKHOA', 'ADMIN')) THEN
            RETURN '1=1';
        ELSE
            RETURN 'MANV = ''' || USER || '''';
        END IF;  
...
DECLARE
    USER VARCHAR2(30);
BEGIN
    USER := sys_context('my_context', 'vpd_bypass');
    DBMS_OUTPUT.PUT_LINE('Role: ' || USER);
END;
/
BEGIN
	dbms_rls.add_policy(
...
GRANT SELECT ON ADMIN.DANGKY TO RL_GIAOVU;
-------PHANCONG
...
CREATE OR REPLACE VIEW ADMIN.UV_TRGKHOA_DANGKY
AS
    SELECT * 
    FROM ADMIN.DANGKY
...
/
GRANT SELECT ON ADMIN.UV_TRGKHOA_DANGKY TO RL_TRUONGKHOA; 
